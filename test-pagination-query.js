...
import { getFirestore, collection, query, limit, getDocs, orderBy, where } from "firebase/firestore";
...
async function test() {
  try {
    const q = query(collection(db, 'posts'), orderBy('publishedAt', 'desc'), limit(20));
    const snap = await getDocs(q);
    console.log("Count docs:", snap.docs.length);
  } catch(e) {
