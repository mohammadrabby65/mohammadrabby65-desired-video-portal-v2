...
import { initializeFirestore, collection, getDocs, query, limit, orderBy, where } from "firebase/firestore";
...
async function main() {
  const allPosts = await getDocs(query(collection(db, "posts"), orderBy("publishedAt", "desc")));
  allPosts.forEach(d => {
    if (d.data().title.includes("Test New Video")) {
