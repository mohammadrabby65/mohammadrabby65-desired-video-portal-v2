...
export function DeadUrls() {
  ...
  const checkUrl = async (video: ScanResult): Promise<ScanResult> => {
    ...
    try {
      ...
      await setDoc(doc(db, "deadUrlCheckHistory", video.id), historyData, {
        merge: true,
      });
    } catch (e) {
  ...
  const processBatch = async () => {
    ...
    const newResults = resultsRef.current.map((r) =>
      batch.find((b) => b.id === r.id)
        ? { ...r, status: "scanning" as UrlStatus }
        : r,
    );
    ...
    const worker = async () => {
      while (
        ...
        setResultsState(
          resultsRef.current.map((r) => (r.id === result.id ? result : r)),
        );
      }
    };
  ...
  const exportCSV = () => {
    const deadUrls = resultsRef.current.filter((r) =>
  ...
  const checkedCount = results.filter(
    (r) => r.status !== "pending" && r.status !== "scanning",
  ).length;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-400 text-sm mt-1">
            Scan your video database for broken or dead external links.
          </p>
        </div>
              ...
              <div className="flex flex-wrap gap-4 pt-2">
                <div className="px-4 py-2 bg-neutral-800 rounded-lg">
                  ...
                  <div className="text-lg font-semibold text-green-500">
                    {results.filter((r) => r.status === "working").length}
                  </div>
                </div>
                <div className="px-4 py-2 bg-neutral-800 rounded-lg">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-neutral-800">
                    {results
                      .filter(
                        (r) => r.status !== "pending" || isScanning || isPaused,
                      )
                      .slice(0, 100)
                      .map((result) => (
                        <tr
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 w-full max-w-lg space-y-4">
            ...
            <div className="flex gap-4">
              <img
                src={editingResult.thumbnailUrl}
                alt=""
              ...
              <div>
                ...
                <p className="text-neutral-400 text-sm mt-1">
                  Current URL status:{" "}
                  <span className="capitalize">{editingResult.status}</span>
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              <button
                ...
                disabled={
                  !editedUrl ||
                  editedUrl === editingResult.videoUrl ||
                  isTestingUrl
                }
              ...
              <button
                onClick={async () => {
                  if (
                    !testResult ||
                    !["working", "redirect"].includes(testResult.status)
                  )
                    return;
                  setIsSavingUrl(true);
                  try {
                    ...
                    setResultsState(
                      resultsRef.current.map((r) =>
                        r.id === editingResult.id ? testResult : r,
                      ),
                    );
                    setEditingResult(null);
                    setTestResult(null);
