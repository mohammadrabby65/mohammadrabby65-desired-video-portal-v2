...
export function Search() {
  ...
  const { data: popularCategories = [] } = usePublicCategories(
    ...
    useQuery({
      ...
      queryFn: async () => {
        ...
        const res = await fetch(
          `/api/videos?q=${encodeURIComponent(debouncedQuery)}&limitCount=20`,
        );
        if (!res.ok) return [];
        ...
        data.videos.forEach((v: any) => {
          const titleMatches = queryWords.every(
            (kw) => v.title && v.title.toLowerCase().includes(kw),
          );
          if (titleMatches && v.title) {
            keywords.add(v.title);
          }
          v.tags?.forEach((t: string) => {
            const tagMatches = queryWords.every((kw) =>
              t.toLowerCase().includes(kw),
            );
            if (tagMatches) keywords.add(t);
          });
          v.categories?.forEach((c: string) => {
            const catMatches = queryWords.every((kw) =>
  ...
  return (
    <div className="flex-1 flex flex-col w-full animate-fade-in">
      <SEO
        title={
          queryTextUrl
            ? `Search results for "${queryTextUrl}" - DesiredHub`
            : "Search - DesiredHub"
        }
        description={
          queryTextUrl
            ? `Browse search results for "${queryTextUrl}" on DesiredHub.`
            : "Search for premium videos on DesiredHub."
        }
            ...
            {queryText.trim() ? (
                    ...
                    {suggestions.map((keyword: string, i: number) => (
                      <li key={i}>
                        ...
                        >
                          ...
                          <span className="text-[15px] font-medium text-neutral-100 line-clamp-1">
                            {keyword}
                          </span>
                        </button>
                      </li>
              ...
              <div className="space-y-8">
                ...
                {popularCategories.length > 0 && (
                  <section>
                    <h3 className="text-sm font-bold text-neutral-100 mb-4 px-2">
                      Popular Categories
                    </h3>
