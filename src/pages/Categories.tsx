  ...
  return (
    <div className="flex-1 pb-20 pt-8 sm:pt-10">
      ...
      <div className="container mx-auto px-4">
        <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-8 tracking-tight">
          Categories
        </h1>
        ...
        {isLoading ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="aspect-[4/3] rounded-2xl bg-neutral-900 "
              />
            ))}
          </div>
        ) : error ? (
            ...
            {categories?.map((cat) => (
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 p-4">
                  <h3 className="text-white font-bold text-lg mb-1 drop-shadow-md group-hover:text-primary ">
                    {cat.name}
                  </h3>
                </div>
              </Link>
