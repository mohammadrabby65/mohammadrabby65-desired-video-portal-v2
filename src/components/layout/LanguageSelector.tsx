  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 rounded-full bg-neutral-900/80 backdrop-blur-md border border-neutral-800 hover:border-primary/50 transition-colors duration-300 text-sm sm:text-base group"
        aria-label="Select language"
      >
        <Globe className="w-4 h-4 sm:w-5 sm:h-5 text-neutral-400 group-hover:text-primary transition-colors" />
        <span className="hidden sm:inline font-medium text-neutral-200">{currentLang.flag} {currentLang.label}</span>
        <span className="sm:hidden font-medium text-neutral-200">{currentLang.flag}</span>
