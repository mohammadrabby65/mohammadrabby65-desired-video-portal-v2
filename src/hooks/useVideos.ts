    ...
    queryFn: async ({
      ...
      if (filter.category && filter.category !== "All") {
        if (filter.category === "Trending") {
          ...
          constraints.push(
            where("categories", "array-contains", filter.category),
          );
  ...
  return useQuery({
    ...
    initialData: () => {
      if (
        ...
        (window as any).__INITIAL_VIDEO_DATA__
      ) {
  ...
  return useInfiniteQuery({
    ...
    initialData: () => {
      ...
      cached.forEach(([_, data]: any) => {
        ...
        if (data.pages)
          data.pages.forEach(
            (page: any) => page.videos && allVideos.push(...page.videos),
          );
...
export function useAdjacentVideos(
  publishedAt: any,
...
export function usePaginationVideos(
  ...
  return useQuery({
    queryKey: [
      ...
      limitCount,
      page,
      filter.sortBy === "random" ? globalRandomSeed : null,
    ],
    queryFn: async () => {
      const params = new URLSearchParams();
