...
const replacement = `<div className="flex w-full max-w-full overflow-x-auto overflow-y-hidden gap-3 sm:gap-4 pb-4 pt-2 px-1 scrollbar-hide snap-x snap-mandatory">
        {images.map((img, idx) => (
          <button
            key={idx}
...
const target = code.match(/<div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 pb-4 pt-2 px-1">[\s\S]*?<\/div>/)[0];
